...
public class ComponentMover extends MouseAdapter {
	...
	private void setupForDragging(MouseEvent e) {
		...
		System.err.println("pressed location x=" + pressed.x + ", y="
				+ pressed.y);

		location = destination.getLocation();
		System.err.println("movable widget location x=" + location.x + ", y="
				+ location.y);
	...
	public void mouseDragged(MouseEvent e) {
		...
		Rectangle virtualDeviceRect = getVirtualDeviceRectangle();

		int locationX = (location.x - virtualDeviceRect.x) + dragX;
		int locationY = (location.y - virtualDeviceRect.y) + dragY;

		// Mouse dragged events are not generated for every pixel the mouse
		// is moved. Adjust the location to make sure we are still on a
		// snap value.

		Dimension bounds = getBoundingSize(destination);
		System.err.println("Bounding size height=" + bounds.height + ", width="
				+ bounds.width);
		...
		while (locationX + destination.getSize().width + edgeInsets.right > bounds.width)
			locationX -= snapSize.width;

		while (locationY + destination.getSize().height + edgeInsets.bottom > bounds.height)
			locationY -= snapSize.height;

		// Adjustments are finished, move the component

		destination.setLocation(getVirtualDeviceRectangle().x + locationX,
				getVirtualDeviceRectangle().y + locationY);
	}
	...
	private Dimension getBoundingSize(Component source) {
		if (source instanceof Window) {
			Rectangle graphicsRect = getVirtualDeviceRectangle();
			return new Dimension(graphicsRect.width, graphicsRect.height);
		} else {
			return source.getParent().getSize();
		}
	}

	private Rectangle getVirtualDeviceRectangle() {
		Rectangle virtualDeviceBounds = new Rectangle();

		GraphicsEnvironment env = GraphicsEnvironment
				.getLocalGraphicsEnvironment();
		for (GraphicsDevice device : env.getScreenDevices()) {
			for (GraphicsConfiguration gc : device.getConfigurations()) {
				virtualDeviceBounds.add(gc.getBounds());
			}
		}

		return virtualDeviceBounds;
	}
